...
public class Crop : MonoBehaviour
{
    ...
    public void ProcessToolAction(ItemDetails tool, TileDetails tile)
    {
        ...
        if (harvestActionCount < requireActionCount)
        {
            ...
            if (cropDetails.hasParticalEffect)
                EventHeadler.CallParticleEffectEvent(cropDetails.effectType, transform.position + cropDetails.effectPos);
            //        //播放声音
        ...
        if (harvestActionCount >= requireActionCount)
        {
            ...
            else if (cropDetails.hasAnimation)
            {
                ...
                //EventHeadler.CallPlaySoundEvent(SoundName.TreeFalling);
    ...
    private void CreateTransferCrop()
    {
        ...
        EventHeadler.CallRefreshCurrentMap();
    }
...
public void SpawnHarvestItems()
    {
        for (int i = 0; i < cropDetails.producedItemID.Length; i++)
        {
            ...
            for (int j = 0; j < amountToProduce; j++)
            {
                ...
                {
                    EventHeadler.CallHarvestAtPlayerPosition(cropDetails.producedItemID[i]);
                }
                else    //世界地图上生成物品
                {
                    //判断应该生成的物品方向xc 
                    ...
                    EventHeadler.CallInstantiateItemInScene(cropDetails.producedItemID[i], spawnPos);
                }
            }
        }
